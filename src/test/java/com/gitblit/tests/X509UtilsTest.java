...
public class X509UtilsTest extends GitblitUnitTest {
	...
	public void prepare() throws Exception {
		cleanUp();
		final X509Metadata goMetadata = new X509Metadata("localhost", this.caPassword);
		X509Utils.prepareX509Infrastructure(goMetadata, this.folder, this.log);
	}

	@After
	public void cleanUp() throws Exception {
		if (this.folder.exists()) {
			FileUtils.delete(this.folder, FileUtils.RECURSIVE);
		}
	}

	@Test
	public void testNewCA() {
		final File storeFile = new File(this.folder, X509Utils.CA_KEY_STORE);
		X509Utils.getPrivateKey(X509Utils.CA_ALIAS, storeFile, this.caPassword);
		final X509Certificate cert = X509Utils.getCertificate(X509Utils.CA_ALIAS, storeFile,
				this.caPassword);
		assertEquals("O=Gitblit,OU=Gitblit,CN=Gitblit Certificate Authority", cert.getIssuerDN()
				.getName());
	}

	@Test
	public void testCertificateUserMapping() {
		final File storeFile = new File(this.folder, X509Utils.CA_KEY_STORE);
		final PrivateKey caPrivateKey = X509Utils.getPrivateKey(X509Utils.CA_ALIAS, storeFile,
				this.caPassword);
		final X509Certificate caCert = X509Utils.getCertificate(X509Utils.CA_ALIAS, storeFile,
				this.caPassword);

		final X509Metadata userMetadata = new X509Metadata("james", "james");
		...
		final X509Certificate cert1 = X509Utils.newClientCertificate(userMetadata, caPrivateKey,
				caCert, storeFile.getParentFile());
		final UserModel userModel1 = HttpUtils.getUserModelFromCertificate(cert1);
		...
		final X509Certificate cert2 = X509Utils.newClientCertificate(userMetadata, caPrivateKey,
				caCert, storeFile.getParentFile());
		final UserModel userModel2 = HttpUtils.getUserModelFromCertificate(cert2);
		...
		assertNotSame("Serial numbers are the same!", cert1.getSerialNumber().longValue(), cert2
	...
	public void testUserBundle() throws Exception {
		...
		final File zip = X509Utils.newClientBundle(userMetadata, storeFile, this.caPassword,
				this.log);
		assertTrue(zip.exists());

		final List<String> expected = Arrays.asList(userMetadata.commonName + ".pem",
				userMetadata.commonName + ".p12", userMetadata.commonName + ".cer", "ca.cer",
				"README.TXT");
	...
	public void testCertificateRevocation() {
		...
		final File caRevocationList = new File(this.folder, X509Utils.CA_REVOCATION_LIST);
		assertFalse(X509Utils.isRevoked(cert1, caRevocationList));

		// revoke certificate and then confirm it IS revoked
		X509Utils.revoke(cert1, RevocationReason.ACompromise, caRevocationList, storeFile,
				this.caPassword, this.log);
		...
		final X509Certificate cert3 = X509Utils.newClientCertificate(userMetadata, caPrivateKey,
				caCert, storeFile.getParentFile());
