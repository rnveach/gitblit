...
/**
 * Creates and deletes a range of ticket references via ticket comments and
 * commits
 */
public class TicketReferenceTest extends GitblitUnitTest {

	static File workingCopy = new File(GitBlitSuite.REPOSITORIES,
			"working/TicketReferenceTest.git-wc");
	...
	public static void configure() throws Exception {
		final File repositoryName = new File("TicketReferenceTest.git");
		;
		...
		final Iterable<PushResult> results = git.push().setPushAll().setCredentialsProvider(cp)
				.call();
	...
	@Test
	public void noReferencesOnTicketCreation() {
	...
	@Test
	public void commentNoUnexpectedReference() {

		TicketModel a = ticketService.createTicket(repo,
				newTicket("commentNoUnexpectedReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commentNoUnexpectedReference-B"));

		assertNotNull(ticketService.updateTicket(repo, a.number,
				newComment("comment for 1 - no reference")));
		assertNotNull(ticketService.updateTicket(repo, a.number,
				newComment("comment for # - no reference")));
		assertNotNull(ticketService.updateTicket(repo, a.number,
	...
	@Test
	public void commentNoSelfReference() {

		TicketModel a = ticketService.createTicket(repo, newTicket("commentNoSelfReference-A"));

		final Change comment = newComment(String.format("comment for #%d - no self reference",
				a.number));
	...
	@Test
	public void commentSingleReference() {
		...
		final Change comment = newComment(String.format("comment for #%d - single reference",
				b.number));
		assertNotNull(ticketService.updateTicket(repo, a.number, comment));
		...
		final List<Reference> cRefB = b.getReferences();
		assertNotNull(cRefB);
	...
	@Test
	public void commentSelfAndOtherReference() {
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commentSelfAndOtherReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commentSelfAndOtherReference-B"));

		final Change comment = newComment(String.format(
				"comment for #%d and #%d - self and other reference", a.number, b.number));
		assertNotNull(ticketService.updateTicket(repo, a.number, comment));
	...
	@Test
	public void commentMultiReference() {
		...
		final Change comment = newComment(String.format(
				"comment for #%d and #%d - multi reference", b.number, c.number));
		assertNotNull(ticketService.updateTicket(repo, a.number, comment));
		...
		final List<Reference> cRefC = c.getReferences();
		assertNotNull(cRefC);
	...
	public void commitMasterSingleReference() throws Exception {
		TicketModel a = ticketService
				.createTicket(repo, newTicket("commitMasterSingleReference-A"));
		...
		final List<Reference> cRefA = a.getReferences();
		assertNotNull(cRefA);
	...
	public void commitMasterMultiReference() throws Exception {
		...
		final String message = String.format("commit for #%d and #%d - multi reference", a.number,
				b.number);
		final RevCommit revCommit1 = makeCommit(message);
		final String commit1Sha = revCommit1.name();
		...
		final List<Reference> cRefB = a.getReferences();
		assertNotNull(cRefB);
		assertEquals(1, cRefB.size());
	...
	public void commitPatchsetNoUnexpectedReference() throws Exception {
		...
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitPatchsetNoUnexpectedReference-A"));

		final String branchName = String.format("ticket/%d", a.number);
	...
	public void commitPatchsetNoSelfReference() throws Exception {
		setPatchsetAvailable(true);
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitPatchsetNoSelfReference-A"));
	...
	public void commitPatchsetSingleReference() throws Exception {
		setPatchsetAvailable(true);
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitPatchsetSingleReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commitPatchsetSingleReference-B"));

		final String branchName = String.format("ticket/%d", a.number);
		git.checkout().setCreateBranch(true).setName(branchName).call();

		final String message = String
				.format("commit for #%d - patchset single reference", b.number);
		final RevCommit revCommit1 = makeCommit(message);
		final String commit1Sha = revCommit1.name();
	...
	public void commitPatchsetMultiReference() throws Exception {
		setPatchsetAvailable(true);
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitPatchsetMultiReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commitPatchsetMultiReference-B"));
		TicketModel c = ticketService.createTicket(repo,
				newTicket("commitPatchsetMultiReference-C"));

		final String branchName = String.format("ticket/%d", a.number);
		git.checkout().setCreateBranch(true).setName(branchName).call();

		final String message = String.format("commit for #%d and #%d- patchset multi reference",
				b.number, c.number);
		final RevCommit revCommit1 = makeCommit(message);
		final String commit1Sha = revCommit1.name();
	...
	public void commitPatchsetAmendReference() throws Exception {
		...
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitPatchsetAmendReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commitPatchsetAmendReference-B"));
		TicketModel c = ticketService.createTicket(repo,
				newTicket("commitPatchsetAmendReference-C"));
		assertFalse(c.hasPatchsets());

		final String branchName = String.format("ticket/%d", c.number);
		git.checkout().setCreateBranch(true).setName(branchName).call();
	...
	public void commitTicketBranchNoUnexpectedReference() throws Exception {
		...
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitTicketBranchNoUnexpectedReference-A"));
	...
	public void commitTicketBranchSelfReference() throws Exception {
		setPatchsetAvailable(false);
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitTicketBranchSelfReference-A"));
	...
	public void commitTicketBranchSingleReference() throws Exception {
		setPatchsetAvailable(false);
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitTicketBranchSingleReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commitTicketBranchSingleReference-B"));
	...
	public void commitTicketBranchMultiCommit() throws Exception {
		setPatchsetAvailable(false);
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitTicketBranchMultiCommit-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commitTicketBranchMultiCommit-B"));
	...
	public void commitTicketBranchMultiReference() throws Exception {
		setPatchsetAvailable(false);
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitTicketBranchMultiReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commitTicketBranchMultiReference-B"));
		TicketModel c = ticketService.createTicket(repo,
				newTicket("commitTicketBranchMultiReference-C"));
	...
	public void commitTicketBranchAmendReference() throws Exception {
		...
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitTicketBranchAmendReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commitTicketBranchAmendReference-B"));
		TicketModel c = ticketService.createTicket(repo,
				newTicket("commitTicketBranchAmendReference-C"));
		assertFalse(c.hasPatchsets());
	...
	public void commitTicketBranchDeleteNoMergeReference() throws Exception {
		...
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitTicketBranchDeleteNoMergeReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commitTicketBranchDeleteNoMergeReference-B"));
		TicketModel c = ticketService.createTicket(repo,
				newTicket("commitTicketBranchDeleteNoMergeReference-C"));
		assertFalse(c.hasPatchsets());

		final String branchName = String.format("ticket/%d", c.number);
		git.checkout().setCreateBranch(true).setName(branchName).call();

		final String message = String.format("commit before amend for #%d and #%d", a.number,
				b.number);
		final RevCommit revCommit1 = makeCommit(message);
		final String commit1Sha = revCommit1.name();
		assertPushSuccess(commit1Sha, branchName);
	...
	public void commitTicketBranchDeletePostMergeReference() throws Exception {
		...
		TicketModel a = ticketService.createTicket(repo,
				newTicket("commitTicketBranchDeletePostMergeReference-A"));
		TicketModel b = ticketService.createTicket(repo,
				newTicket("commitTicketBranchDeletePostMergeReference-B"));
		TicketModel c = ticketService.createTicket(repo,
				newTicket("commitTicketBranchDeletePostMergeReference-C"));
		assertFalse(c.hasPatchsets());
	...
	private static RevCommit makeCommit(String message) throws Exception {
		final File file = new File(workingCopy, "testFile.txt");
		...
		final RevCommit rev = git.commit().setMessage(message).call();
		return rev;
	}
	...
	private static String amendCommit(String message) throws Exception {
		...
		w.close();
		git.add().addFilepattern(file.getName()).call();
		final RevCommit rev = git.commit().setAmend(true).setMessage(message).call();
		return rev.getId().name();
	}

	private static void setPatchsetAvailable(boolean state) throws GitBlitException {
		repo.acceptNewPatchsets = state;
		gitblit().updateRepositoryModel(repo.name, repo, false);
	}

	private static void assertPushSuccess(String commitSha, String branchName) throws Exception {
		final Iterable<PushResult> results = git.push().setRemote("origin")
				.setCredentialsProvider(cp).call();

		for (final PushResult result : results) {
			final RemoteRefUpdate ref = result.getRemoteUpdate("refs/heads/" + branchName);
			assertEquals(Status.OK, ref.getStatus());
			assertEquals(commitSha, ref.getNewObjectId().name());
		}
	}

	private static void assertForcePushSuccess(String commitSha, String branchName)
			throws Exception {
		final Iterable<PushResult> results = git.push().setForce(true).setRemote("origin")
				.setCredentialsProvider(cp).call();
	...
	private static void assertDeleteBranch(String branchName) throws Exception {

		final RefSpec refSpec = new RefSpec().setSource(null).setDestination(
				"refs/heads/" + branchName);

		final Iterable<PushResult> results = git.push().setRefSpecs(refSpec).setRemote("origin")
				.setCredentialsProvider(cp).call();
