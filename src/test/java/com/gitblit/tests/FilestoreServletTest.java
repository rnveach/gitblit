...
public class FilestoreServletTest extends GitblitUnitTest {
	...
	public void testRegexGroups() {
		...
		final Pattern p = Pattern.compile(FilestoreServlet.REGEX_PATH);

		final String basicUrl = "https://localhost:8080/r/test.git/info/lfs/objects/";
		final String batchUrl = basicUrl + "batch";
		final String oidUrl = basicUrl + SHA256_EG;
	...
	@Test
	public void testRegexGroupsNestedRepo() {

		final Pattern p = Pattern.compile(FilestoreServlet.REGEX_PATH);

		final String basicUrl = "https://localhost:8080/r/nested/test.git/info/lfs/objects/";
		final String batchUrl = basicUrl + "batch";
		final String oidUrl = basicUrl + SHA256_EG;
	...
	public void testDownload() throws Exception {
		...
		final RepositoryModel r = gitblit().getRepositoryModel(repoName);
		...
		// Emulate a pre-existing Git-LFS repository by using using internal
		// pre-tested methods
		assertEquals(
				Status.Available,
				filestore().uploadBlob(blob.hash, blob.length, u, r,
						new ByteArrayInputStream(blob.blob)));
		...
		final String expectedContent = String.format("{%s:%s,%s:%d,%s:{%s:{%s:%s}}}", "\"oid\"",
				"\"" + blob.hash + "\"", "\"size\"", blob.length, "\"actions\"", "\"download\"",
				"\"href\"", "\"" + downloadURL + "\"");
		...
		final byte[] dlData = IOUtils.toByteArray(response.getEntity().getContent());
	...
	public void testDownloadMultiple() throws Exception {
		...
		final HttpClient client = HttpClientBuilder.create().build();
		final HttpPost request = new HttpPost(batchURL);

		// add request header
		request.addHeader(HttpHeaders.ACCEPT, FilestoreServlet.GIT_LFS_META_MIME);
		request.addHeader(HttpHeaders.CONTENT_ENCODING, FilestoreServlet.GIT_LFS_META_MIME);

		final String content = String.format("{%s:%s,%s:[{%s:%s,%s:%d},{%s:%s,%s:%d}]}",
				"\"operation\"", "\"download\"", "\"objects\"", "\"oid\"", "\"" + blob.hash + "\"",
				"\"size\"", blob.length, "\"oid\"", "\"" + SHA256_EG + "\"", "\"size\"", 0);

		final HttpEntity entity = new ByteArrayEntity(content.getBytes("UTF-8"));
		request.setEntity(entity);

		final HttpResponse response = client.execute(request);

		final String responseMessage = IOUtils.toString(response.getEntity().getContent(), "UTF-8");
		assertEquals(200, response.getStatusLine().getStatusCode());

		final String expectedContent = String.format(
				"{%s:[{%s:%s,%s:%d,%s:{%s:{%s:%s}}},{%s:%s,%s:%d,%s:{%s:%s,%s:%d}}]}",
				"\"objects\"", "\"oid\"", "\"" + blob.hash + "\"", "\"size\"", blob.length,
				"\"actions\"", "\"download\"", "\"href\"", "\"" + downloadURL + "\"", "\"oid\"",
				"\"" + SHA256_EG + "\"", "\"size\"", 0, "\"error\"", "\"message\"",
				"\"Object not available\"", "\"code\"", 404);
	...
	public void testDownloadUnavailable() throws Exception {
		...
		final String expectedError = String.format("{%s:%s,%s:%d}", "\"message\"",
				"\"Object not available\"", "\"code\"", 404);
	...
	public void testUpload() throws Exception {
		...
		final HttpClient client = HttpClientBuilder.create().build();
		final HttpPost request = new HttpPost(initialUploadURL);

		// add request header
		request.addHeader(HttpHeaders.ACCEPT, FilestoreServlet.GIT_LFS_META_MIME);
		request.addHeader(HttpHeaders.CONTENT_ENCODING, FilestoreServlet.GIT_LFS_META_MIME);

		final String content = String.format("{%s:%s,%s:[{%s:%s,%s:%d}]}", "\"operation\"",
				"\"upload\"", "\"objects\"", "\"oid\"", "\"" + blob.hash + "\"", "\"size\"",
				blob.length);
		...
		final String expectedContent = String.format("{%s:[{%s:%s,%s:%d,%s:{%s:{%s:%s}}}]}",
				"\"objects\"", "\"oid\"", "\"" + blob.hash + "\"", "\"size\"", blob.length,
				"\"actions\"", "\"upload\"", "\"href\"", "\"" + expectedUploadURL + "\"");

		assertEquals(expectedContent, responseMessage);

		// Now try to upload the binary download
		final HttpPut putRequest = new HttpPut(expectedUploadURL);
		...
		final ByteArrayOutputStream savedBlob = new ByteArrayOutputStream();
	...
	public void testMalformedUpload() throws Exception {
		...
		final String content = String.format("{%s:%s,%s:[{%s:%s,%s,%d}]}", "operation", "upload",
				"objects", "oid", blob.hash, "size", blob.length);
		...
		final String expectedError = String.format("{%s:%s,%s:%d}", "\"message\"",
				"\"Malformed Git-LFS request\"", "\"code\"", 400);
