...
/**
 * Simple helper class to hide some common setup needed to use JSoup as an HTML
 * generator. A {@link HtmlBuilder} has a root element that can be manipulated
 * in all the usual JSoup ways (but not added to some {@link Document}); to
 * generate HTML for that root element, the builder's {@link #toString()} method
 * can be used. (Or one can invoke toString() directly on the root element.) By
 * default, a HTML builder does not pretty-print the HTML.
 *
...
public class HtmlBuilder {
	...
	/**
	 * Creates a new HTML builder with a root element with the given
	 * {@code tagName}.
	 *
	 * @param tagName
	...
	/**
	 * @return whether this HTML builder will pretty-print the HTML generated by
	 *         {@link #toString()}
	 */
	...
	/**
	 * Sets whether this HTML builder will produce pretty-printed HTML in its
	...
	public HtmlBuilder prettyPrint(boolean pretty) {
		this.shell.outputSettings().prettyPrint(pretty);
		return this;
	}
