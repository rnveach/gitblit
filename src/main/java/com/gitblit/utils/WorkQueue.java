...
public class WorkQueue {
	...
	private static final UncaughtExceptionHandler LOG_UNCAUGHT_EXCEPTION = new UncaughtExceptionHandler() {
		@Override
	...
	public synchronized Executor getDefaultQueue() {
		if (this.defaultQueue == null) {
			this.defaultQueue = createQueue(this.defaultQueueSize, "WorkQueue");
		}
	...
	public List<Task<?>> getTasks() {
		...
		for (final Executor e : this.queues) {
			e.addAllTo(r);
		}
		return r;
	}
	...
	public <T> List<T> getTaskInfos(TaskInfoFactory<T> factory) {
		final List<T> taskInfos = Lists.newArrayList();
		for (final Executor exe : this.queues) {
	...
	public Task<?> getTask(final int id) {
		...
		for (final Executor e : this.queues) {
			...
			if (t != null) {
				if (result != null) {
					// Don't return the task if we have a duplicate. Lie
					// instead.
					return null;
				} else {
					result = t;
	...
	public void stop() {
		for (final Executor p : this.queues) {
			p.shutdown();
	...
	public class Executor extends ScheduledThreadPoolExecutor {
		...
		Executor(final int corePoolSize, final String prefix) {
			super(corePoolSize, new ThreadFactory() {
				...
				public Thread newThread(final Runnable task) {
					final Thread t = this.parent.newThread(task);
			...
			this.all = new ConcurrentHashMap<Integer, Task<?>>( //
		...
		protected <V> RunnableScheduledFuture<V> decorateTask(final Runnable runnable,
				RunnableScheduledFuture<V> r) {
			r = super.decorateTask(runnable, r);
			for (;;) {
				final int id = WorkQueue.this.idGenerator.next();

				Task<V> task;
				task = new Task<V>(runnable, r, this, id);

				if (this.all.putIfAbsent(task.getTaskId(), task) == null) {
					return task;
				}
			}
		}
		...
		void addAllTo(final List<Task<?>> list) {
			list.addAll(this.all.values()); // iterator is thread safe
		}
