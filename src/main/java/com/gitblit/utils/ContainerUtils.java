...
public class ContainerUtils {
	...
	public static class CVE_2007_0450 {
		...
		public static void test(IStoredSettings settings) {
			final boolean mounted = settings.getBoolean(Keys.web.mountParameters, true);
			final char fsc = settings.getChar(Keys.web.forwardSlashCharacter, '/');
			if (mounted && ((fsc == '/') || (fsc == '\\'))) {
		...
		private static boolean logCVE_2007_0450Tomcat(IStoredSettings settings) {
			try {
				final byte[] test = "http://server.domain:8080/context/servlet/param%2fparam"
						.getBytes();

				// ByteChunk mb=new ByteChunk();
				final Class<?> cByteChunk = Class.forName("org.apache.tomcat.util.buf.ByteChunk");
				final Object mb = cByteChunk.newInstance();

				// mb.setBytes(test, 0, test.length);
				final Method setBytes = cByteChunk.getMethod("setBytes", byte[].class, int.class,
						int.class);
				setBytes.invoke(mb, test, 0, test.length);

				// UDecoder ud=new UDecoder();
				final Class<?> cUDecoder = Class.forName("org.apache.tomcat.util.buf.UDecoder");
				final Object ud = cUDecoder.newInstance();

				// ud.convert(mb,false);
				final Method convert = cUDecoder.getMethod("convert", cByteChunk, boolean.class);
				...
				try {
					...
					if ((e.getTargetException() != null)
							&& (e.getTargetException() instanceof CharConversionException)) {
						...
						LOGGER.warn("Overriding {} and setting to {}",
