...
public class JnaUtils {
	...
	public static final int S_ISVTX = 0001000; // sticky bit, save swapped text
												// even after use
	...
	public static final int S_IXUSR = 0000100; // execute/search permission for
												// owner
	...
	public static final int S_IXGRP = 0000010; // execute/search permission for
												// group
	...
	public static final int S_IXOTH = 0000001; // execute/search permission for
												// other
	 ...
	 * @return Upon successful completion, a value of 0 returned. Otherwise, a
	 ...
	 *
	 * This method is only implemented for OSes of the Unix family. It returns
	 * the file mode information as available in the st_mode member of the
	 * resulting struct stat when calling 'lstat' on a file.
	 *
	 * @param path
	 *            File/directory to get the file mode from.
	 * @return Upon successful completion, the file mode bits are returned.
	...
	public static int getFilemode(String path) {
		...
		final Filestat stat = getFilestat(path);
		if (stat == null) {
			return -1;
		}
		return stat.mode;
	 ...
	 *
	 * This method is only implemented for OSes of the Unix family. It returns
	 * file status information for a file. Currently this is the file mode, the
	 * user id and group id of the owner.
	 *
	 * @param path
	 *            File/directory to get the file status from.
	 * @return Upon successful completion, a Filestat object containing the file
	...
	public static Filestat getFilestat(String path) {
		...
		// Use a Runtime, because implementing stat() via JNA is just too much
		// trouble.
		// This could be done with the 'stat' command, too. But that may have a
		// shell specific implementation, so we use 'ls' instead.
		final String lsLine = runProcessLs(path);
		if (lsLine == null) {
		...
		final Matcher m = p.matcher(lsLine);
		if (!m.lookingAt()) {
	...
	/**
	 * Run the unix command 'ls -ldn' on a single file and return the resulting
	 * output line.
	 *
	 * @param path
	 *            Path to a single file or directory.
	 * @return The first line of output from the 'ls' command. Null, if an error
	 *         occurred and no line could be read.
	 */
	private static String runProcessLs(String path) {
		final String cmd = "ls -ldn " + path;
