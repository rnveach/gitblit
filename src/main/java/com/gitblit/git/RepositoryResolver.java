...
public class RepositoryResolver<X> extends FileResolver<X> {
	 ...
	 */
	@Override
	public Repository open(final X req, final String name) throws RepositoryNotFoundException,
			ServiceNotEnabledException {
		final Repository repo = super.open(req, name);
	...
	/**
	 * Check if this repository can be served by the requested client
	 * connection.
	 */
	@Override
	protected boolean isExportOk(X req, String repositoryName, Repository db) throws IOException {
		final RepositoryModel model = this.gitblit.getRepositoryModel(repositoryName);
		...
		if (user.canClone(model)) {
			...
			this.logger.debug(MessageFormat.format(
					"{0}:// access of {1} by {2} from {3} PERMITTED", scheme, repositoryName,
					user.username, origin));
			return true;
		}

		// user can not access this git repo
		this.logger.warn(MessageFormat.format("{0}:// access of {1} by {2} from {3} DENIED",
