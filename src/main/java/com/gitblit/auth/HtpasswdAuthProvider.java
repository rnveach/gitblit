...
/**
 * Implementation of a user service using an Apache htpasswd file for
 * authentication.
 *
 * This user service implement custom authentication using entries in a file
 * created by the 'htpasswd' program of an Apache web server. All possible
 * output options of the 'htpasswd' program version 2.2 are supported: plain
 * text (only on Windows and Netware), glibc crypt() (not on Windows and
 * NetWare), Apache MD5 (apr1), unsalted SHA-1.
 *
 * Configuration options: realm.htpasswd.backingUserService - Specify the
 * backing user service that is used to keep the user data other than the
 * password. The default is '${baseFolder}/users.conf'. realm.htpasswd.userfile
 * - The text file with the htpasswd entries to be used for authentication. The
 * default is '${baseFolder}/htpasswd'.
 * realm.htpasswd.overrideLocalAuthentication - Specify if local accounts are
 * overwritten when authentication matches for an external account.
 *
...
public class HtpasswdAuthProvider extends UsernamePasswordAuthenticationProvider {
	 ...
	 *
	 * The HtpasswdUserService extends the GitblitUserService and is thus backed
	 * by the available user services provided by the GitblitUserService. In
	 * addition the setup tries to read and parse the htpasswd file to be used
	 * for authentication.
	 *
	 * @param settings
	 * @since 0.7.0
	...
	public void setup() {
		...
		read();
		this.logger.debug("Read " + this.htUsers.size() + " users from htpasswd file: "
				+ this.htpasswdFile);
	}
	 ...
	 *
	 * If the account is determined to be a local account, authentication will
	 * be done against the locally stored password. Otherwise, the configured
	 * htpasswd file is read. All current output options of htpasswd are
	 * supported: clear text, crypt(), Apache MD5 and unsalted SHA-1.
	 *
	 * @param username
	 * @param password
	...
	public UserModel authenticate(String username, char[] password) {
		read();
		final String storedPwd = this.htUsers.get(username);
		if (storedPwd != null) {
			...
			// test unsalted SHA password
				...
				final String passwd64 = Base64.encodeBase64String(DigestUtils.sha1(passwd));
				if (storedPwd.substring("{SHA}".length()).equals(passwd64)) {
					this.logger.debug("Unsalted SHA-1 encoded password matched for user '"
							+ username + "'");
					authenticated = true;
				}
			}
			// test libc crypt() encoded password
				...
				this.logger
						.debug("Libc crypt encoded password matched for user '" + username + "'");
				authenticated = true;
			}
			// test clear text
			...
			if (authenticated) {
				...
				final UserModel curr = this.userManager.getUserModel(username);
				UserModel user;
				if (curr == null) {
	...
	private synchronized void read() {
		...
		final File file = getFileOrFolder(KEY_HTPASSWD_FILE, DEFAULT_HTPASSWD_FILE);
		...
		if (this.htpasswdFile.exists()
				&& (forceReload || (this.htpasswdFile.lastModified() != this.lastModified))) {
	...
	/*
	 * Method only used for unit tests. Return number of users read from
	 * htpasswd file.
	 */
	...
	public String toString() {
		return getClass().getSimpleName() + "("
				+ ((this.htpasswdFile != null) ? this.htpasswdFile.getAbsolutePath() : "null")
				+ ")";
	}
}
