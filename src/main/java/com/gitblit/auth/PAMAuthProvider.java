...
public class PAMAuthProvider extends UsernamePasswordAuthenticationProvider {
	...
	public void setup() {
		...
		final String[] files = { "/etc/shadow", "/etc/master.passwd" };
		...
		if (passwdFile == null) {
			...
			this.logger
					.error("PAM Authentication can not read passwd database {}! PAM authentications may fail!",
							passwdFile);
		}
	}
	...
	public UserModel authenticate(String username, char[] password) {
		...
		PAM pam = null;
		try {
			final String serviceName = this.settings.getString(Keys.realm.pam.serviceName,
					"system-auth");
