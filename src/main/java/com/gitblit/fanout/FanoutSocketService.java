...
/**
 * A multi-threaded socket implementation of
 * https://github.com/travisghansen/fanout
 *
 * This implementation creates a master acceptor thread which accepts incoming
 * fanout connections and then spawns a daemon thread for each accepted
 * connection. If there are 100 concurrent fanout connections, there are 101
 * threads.
 * 
 * @author James Moger
 *
...
public class FanoutSocketService extends FanoutService {
	...
	public static void main(String[] args) {
		final FanoutSocketService pubsub = new FanoutSocketService(null, DEFAULT_PORT);
	...
	protected boolean connect() {
		if (this.serviceSocket == null) {
			try {
				...
				this.serviceSocket.setSoTimeout(serviceTimeout);
				this.serviceSocket.bind(this.host == null ? new InetSocketAddress(this.port)
						: new InetSocketAddress(this.host, this.port));
				logger.info(MessageFormat.format("{0} is ready on {1}:{2,number,0}", this.name,
						this.host == null ? "0.0.0.0" : this.host,
	...
	protected void listen() throws IOException {
		try {
			Socket socket;
			socket = this.serviceSocket.accept();
	...
	/**
	 * FanoutSocketConnection handles reading/writing messages from a remote
	 * fanout connection.
	 *
	 * @author James Moger
	 *
	...
	class FanoutSocketConnection extends FanoutServiceConnection implements Runnable {
		...
		public void run() {
			try {
				...
				final byte[] buffer = new byte[FanoutConstants.BUFFER_LENGTH];
				int len = 0;
				while (true) {
				...
				if (t instanceof SocketException) {
					logger.error(MessageFormat.format("fanout connection {0}: {1}", this.id,
							t.getMessage()));
					...
					logger.error(MessageFormat.format(
							"exception while handling fanout connection {0}", this.id), t);
				}
			}
			finally {
				closeConnection();
			...
			logger.info(MessageFormat.format("thread for fanout connection {0} is finished",
					this.id));
		}
		...
		protected void reply(String content) throws IOException {
			...
			final byte[] bytes = content.getBytes(FanoutConstants.CHARSET);
			os.write(bytes);
