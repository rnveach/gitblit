...
public class EditTeamPage extends RootSubPage {
	...
	public EditTeamPage(PageParameters params) {
		...
		super(params);
		this.isCreate = false;
		final String name = WicketUtils.getTeamname(params);
		final TeamModel model = app().users().getTeamModel(name);
		setupPage(model);
		setStatelessHint(false);
		setOutputMarkupId(true);
	}
	...
	protected void setupPage(final TeamModel teamModel) {
		...
		final CompoundPropertyModel<TeamModel> model = new CompoundPropertyModel<TeamModel>(
				teamModel);

		final List<String> repos = getAccessRestrictedRepositoryList(true, null);

		final List<String> teamUsers = new ArrayList<String>(teamModel.users);
		...
		final Palette<UserChoice> users = new Palette<UserChoice>("users",
				...
				new CollectionModel<UserChoice>(sortByDisplayName(getTeamUsers(app().users()
						.getAllUsernames()))), new ChoiceRenderer<UserChoice>(null, "userId"), 10,
				false);
		...
		final Palette<String> preReceivePalette = new Palette<String>("preReceiveScripts",
				new ListModel<String>(preReceiveScripts), new CollectionModel<String>(app()
						.repositories().getPreReceiveScriptsUnused(null)),
		...
		final Palette<String> postReceivePalette = new Palette<String>("postReceiveScripts",
				new ListModel<String>(postReceiveScripts), new CollectionModel<String>(app()
						.repositories().getPostReceiveScriptsUnused(null)),
				new StringChoiceRenderer(), 12, true);

		final Form<TeamModel> form = new Form<TeamModel>("editForm", model) {
			...
			protected void onSubmit() {
				final String teamname = teamModel.name;
				...
				final Iterator<UserChoice> selectedUsers = users.getSelectedChoices();
				...
				// set mailing lists
				final String ml = EditTeamPage.this.mailingLists.getObject();
		...
		final boolean editMemberships = app().authentication().supportsTeamMembershipChanges(
				teamModel);

		// not all user providers support manipulating the admin role
		final boolean changeAdminRole = app().authentication().supportsRoleChanges(teamModel,
				Role.ADMIN);

		// not all user providers support manipulating the create role
		final boolean changeCreateRole = app().authentication().supportsRoleChanges(teamModel,
				Role.CREATE);

		// not all user providers support manipulating the fork role
		final boolean changeForkRole = app().authentication().supportsRoleChanges(teamModel,
				Role.FORK);

		// field names reflective match TeamModel fields
		form.add(new TextField<String>("name"));
		form.add(new CheckBox("canAdmin").setEnabled(changeAdminRole));
		form.add(new CheckBox("canFork").setEnabled(app().settings().getBoolean(
				Keys.web.allowForking, true)
				&& changeForkRole));
		form.add(new CheckBox("canCreate").setEnabled(changeCreateRole));
		form.add(users.setEnabled(editMemberships));
		this.mailingLists = new Model<String>(teamModel.mailingLists == null ? ""
				: StringUtils.flattenStrings(teamModel.mailingLists, " "));
		form.add(new TextField<String>("mailingLists", this.mailingLists));
	...
	private List<UserChoice> getTeamUsers(List<String> teamUserIds) {
		final List<UserChoice> teamUsers = new ArrayList<UserChoice>();
		for (final String teamUserId : teamUserIds) {
			final UserModel userModel = app().users().getUserModel(teamUserId);
			if (userModel != null) {
				teamUsers.add(new UserChoice(userModel.displayName, userModel.username,
	...
	private static List<UserChoice> sortByDisplayName(List<UserChoice> teamUsers) {
