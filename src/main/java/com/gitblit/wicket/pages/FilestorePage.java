...
/**
 * Page to display the current status of the filestore. Certain errors also
 * displayed to aid in fault finding
 *
 * @author Paul Martin
 */
...
public class FilestorePage extends RootPage {
	...
	public FilestorePage(PageParameters params) {
		...
		for (final FilestoreModel file : files) {
		...
		final BookmarkablePageLink<Void> itemOk = new BookmarkablePageLink<Void>("filterByOk",
				FilestorePage.class, WicketUtils.newFilestorePageParameter(prevPage,
						SortBy.ok.name()));

		final BookmarkablePageLink<Void> itemPending = new BookmarkablePageLink<Void>(
				"filterByPending", FilestorePage.class, WicketUtils.newFilestorePageParameter(
						prevPage, SortBy.pending.name()));

		final BookmarkablePageLink<Void> itemInprogress = new BookmarkablePageLink<Void>(
				"filterByInprogress", FilestorePage.class, WicketUtils.newFilestorePageParameter(
						prevPage, SortBy.inprogress.name()));

		final BookmarkablePageLink<Void> itemError = new BookmarkablePageLink<Void>(
				"filterByError", FilestorePage.class, WicketUtils.newFilestorePageParameter(
						prevPage, SortBy.error.name()));

		final BookmarkablePageLink<Void> itemDeleted = new BookmarkablePageLink<Void>(
				"filterByDeleted", FilestorePage.class, WicketUtils.newFilestorePageParameter(
						prevPage, SortBy.deleted.name()));
		...
		if (filter == null) {
			...
			for (final FilestoreModel item : files) {
		...
		final DataView<FilestoreModel> filesView = new DataView<FilestoreModel>("fileRow",
			...
			public void populateItem(final Item<FilestoreModel> item) {
				...
				final DateFormat dateFormater = new SimpleDateFormat(Constants.ISO8601);

				UserModel user = app().users().getUserModel(entry.getChangedBy());
				user = user == null ? UserModel.ANONYMOUS : user;

				final Label icon = FilestoreUI.getStatusIcon("status", entry);
				item.add(icon);
		...
		add(new BookmarkablePageLink<Void>("firstPageBottom", FilestorePage.class)
		...
		itemPending.add(FilestoreUI.getStatusIcon("statusPendingIcon",
				FilestoreModel.Status.Upload_Pending));
		itemInprogress.add(FilestoreUI.getStatusIcon("statusInprogressIcon",
				FilestoreModel.Status.Upload_In_Progress));
		itemError.add(FilestoreUI.getStatusIcon("statusErrorIcon",
				FilestoreModel.Status.Error_Unknown));
		itemDeleted.add(FilestoreUI.getStatusIcon("statusDeletedIcon",
				FilestoreModel.Status.Deleted));
		...
		final BookmarkablePageLink<Void> helpLink = new BookmarkablePageLink<Void>("filestoreHelp",
				FilestoreUsage.class);
	...
	private static class SortableFilestoreProvider extends SortableDataProvider<FilestoreModel> {
		...
		private final List<FilestoreModel> list;
