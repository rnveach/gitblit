...
public class CommitDiffPage extends RepositoryPage {
	...
	public CommitDiffPage(PageParameters params) {
		...
		add(new LinkPanel("whitespaceLink", null, getString(diffComparator.getOpposite()
				.getTranslationKey()), CommitDiffPage.class, WicketUtils.newDiffParameter(
				this.repositoryName, this.objectId, diffComparator.getOpposite())));
		...
		final DiffOutput diff = DiffUtils.getCommitDiff(r, commit, diffComparator,
				DiffOutputType.HTML, handler, tabLength);
		if (handler.getImgDiffCount() > 0) {
			addBottomScript("scripts/imgdiff.js"); // Tiny support script for
													// image diffs
		}
		...
		// git notes
		final List<GitNote> notes = JGitUtils.getNotesOnCommit(r, commit);
		...
		final DataView<GitNote> notesView = new DataView<GitNote>("notes", notesDp) {
			...
			public void populateItem(final Item<GitNote> item) {
				...
				item.add(new RefsPanel("refName", CommitDiffPage.this.repositoryName, Arrays
				...
				item.add(new Label("noteContent", bugtraqProcessor().processPlainCommitMessage(
						getRepository(), CommitDiffPage.this.repositoryName, entry.content))
			...
			public void populateItem(final Item<PathChangeModel> item) {
				...
				if (entry.isTree()) {
					...
					item.add(new LinkPanel("pathName", null, entry.path, TreePage.class,
							WicketUtils.newPathParameter(CommitDiffPage.this.repositoryName,
				...
				if (entry.isSubmodule()) {
					...
					item.add(new BookmarkablePageLink<Void>("patch", PatchPage.class, WicketUtils
							.newPathParameter(CommitDiffPage.this.repositoryName, entry.commitId,
					...
					if (entry.isFilestoreItem()) {
						...
						item.add(new BookmarkablePageLink<Void>("view", BlobPage.class, WicketUtils
								.newPathParameter(CommitDiffPage.this.repositoryName,
										entry.commitId, entry.path)).setEnabled(!entry.changeType
								.equals(ChangeType.DELETE)));

						item.add(new ExternalLink("raw", RawServlet.asLink(getContextUrl(),
								CommitDiffPage.this.repositoryName, entry.commitId, entry.path))
