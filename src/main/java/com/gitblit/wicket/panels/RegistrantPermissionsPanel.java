...
public class RegistrantPermissionsPanel extends BasePanel {
	...
	public RegistrantPermissionsPanel(String wicketId, RegistrantType registrantType,
			List<String> allRegistrants, final List<RegistrantAccessPermission> permissions,
			final Map<AccessPermission, String> translations) {
		super(wicketId);
		 ...
		 */
		final Form<Void> permissionToggleForm = new Form<Void>("permissionToggleForm");
		 ...
		 */
		final RefreshingView<RegistrantAccessPermission> dataView = new RefreshingView<RegistrantAccessPermission>(
				"permissionRow") {
			private static final long serialVersionUID = 1L;

			@Override
			protected Iterator<IModel<RegistrantAccessPermission>> getItemModels() {
				// the iterator returns RepositoryPermission objects, but we
				// need it to
				// return models
				return new ModelIteratorAdapter<RegistrantAccessPermission>(permissions.iterator()) {
					@Override
					protected IModel<RegistrantAccessPermission> model(
							RegistrantAccessPermission permission) {
			...
			public void populateItem(final Item<RegistrantAccessPermission> item) {
				...
				if (RegistrantType.REPOSITORY.equals(entry.registrantType)) {
					final String repoName = StringUtils.stripDotGit(entry.registrant);
					if (!entry.isMissing() && (StringUtils.findInvalidCharacter(repoName) == null)) {
						// repository, strip .git and show swatch
						final Fragment repositoryFragment = new Fragment("registrant",
								"repositoryRegistrant", RegistrantPermissionsPanel.this);
						final Component swatch = new Label("repositorySwatch", "&nbsp;")
								.setEscapeModelStrings(false);
						WicketUtils.setCssBackground(swatch, entry.toString());
						repositoryFragment.add(swatch);
						final Label registrant = new Label("repositoryName", repoName);
						repositoryFragment.add(registrant);
						item.add(repositoryFragment);
					} else {
						// regex or missing
						final Label label = new Label("registrant", entry.registrant);
					...
					final UserModel user = app().users().getUserModel(entry.registrant);
					if (user != null) {
						ident = new PersonIdent(user.getDisplayName(),
								user.emailAddress == null ? user.getDisplayName()
										: user.emailAddress);
					}

					final Fragment userFragment = new Fragment("registrant", "userRegistrant",
					...
					// team
					final Fragment teamFragment = new Fragment("registrant", "teamRegistrant",
							RegistrantPermissionsPanel.this);
				...
				case ADMINISTRATOR:
					final Label administrator = new Label("pType",
							entry.source == null ? getString("gb.administrator") : entry.source);
					WicketUtils.setHtmlTooltip(administrator,
							getString("gb.administratorPermission"));
					WicketUtils.setCssClass(administrator, "label label-inverse");
					item.add(administrator);
					break;
				case OWNER:
					final Label owner = new Label("pType", getString("gb.owner"));
					WicketUtils.setHtmlTooltip(owner, getString("gb.ownerPermission"));
					WicketUtils.setCssClass(owner, "label label-info");
					item.add(owner);
					break;
				case TEAM:
					final Label team = new Label("pType",
							entry.source == null ? getString("gb.team") : entry.source);
					...
					break;
				case REGEX:
					final Label regex = new Label("pType", "regex");
					if (!StringUtils.isEmpty(entry.source)) {
						WicketUtils
								.setHtmlTooltip(regex, MessageFormat.format(
										getString("gb.regexPermission"), entry.source));
					}
				...
				default:
					if (entry.isMissing()) {
						// repository is missing, this permission will be
						// removed on save
						final Label missing = new Label("pType", getString("gb.missing"));
		 ...
		 * Add permission form
		 */
		final IModel<RegistrantAccessPermission> addPermissionModel = new CompoundPropertyModel<RegistrantAccessPermission>(
				new RegistrantAccessPermission(registrantType));
		final Form<RegistrantAccessPermission> addPermissionForm = new Form<RegistrantAccessPermission>(
		...
		addPermissionForm.add(new DropDownChoice<AccessPermission>("permission", Arrays
				.asList(AccessPermission.NEWPERMISSIONS),
				new AccessPermissionRenderer(translations)));
		final AjaxButton button = new AjaxButton("addPermissionButton", addPermissionForm) {
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
				...
				final RegistrantAccessPermission rp = (RegistrantAccessPermission) form.getModel()
						.getObject();
				...
				final RegistrantAccessPermission copy = DeepCopier.copy(rp);
	...
	private class ShowStateButton extends AjaxButton {
		...
		protected void onBeforeRender() {
			String cssClass = "btn";
			if (this.buttonState.equals(RegistrantPermissionsPanel.this.activeState)) {
